...
class UavUdpGateway:
    def __init__(self):
        ...
        # BOOT启动器已经接收并ACK了地面站BOOT，因此新启动的gateway需要
        # 直接继承该任务模式，进入STARTING并在系统真正就绪后上报UAV_BOOT_READY。
        self.startup_boot_task = normalize_task(
            rospy.get_param("~startup_boot_task", "")
        )
        if (
            self.startup_boot_task
            and self.forced_task
            and self.startup_boot_task != self.forced_task
        ):
            raise RuntimeError(
                "startup_boot_task={} conflicts with forced_task={}".format(
                    self.startup_boot_task, self.forced_task
                )
            )
        ...
        self.servo_lock_command = str(
            rospy.get_param("~servo_lock_command", acfg.get("lock_command", "A30"))
        ).strip().upper()
        self.servo_release_command = str(
            rospy.get_param("~servo_release_command", acfg.get("release_command", "A90"))
        ).strip().upper()
        self.servo_result_timeout = max(
            0.30,
            safe_float(
                rospy.get_param(
                    "~servo_result_timeout_s",
                    acfg.get("manual_command_timeout_s", 1.80),
                ),
                1.80,
            ),
        )
        ...
        self.mission_type_pub = rospy.Publisher(
            "/uav/mission_type", String, queue_size=10, latch=True
        )
        self.land_pub = rospy.Publisher("/uav/land", Bool, queue_size=10)
        self.reset_pub = rospy.Publisher("/uav/reset", Bool, queue_size=10)
        # 串口只由 drop_actuator_serial.py 独占；网关和 FSM 都发布到同一话题。
        self.servo_cmd_pub = rospy.Publisher("/uav/drop_cmd", String, queue_size=20)
        ...
        self.boot_task = self.startup_boot_task
        self.boot_state = "STARTING" if self.startup_boot_task else "STOPPED"
        ...
        self.servo_result_condition = threading.Condition()
        self.servo_results = {}
        self.manual_servo_action_id = int(time.time() * 1000) % 1000000000
        self.event_queue = queue.Queue(maxsize=128)
        self.event_stop = threading.Event()

        if self.startup_boot_task:
            startup_mission_type = task_to_mission_type(self.startup_boot_task)
            self.mission_type_pub.publish(String(data=startup_mission_type))
            rospy.logwarn(
                "Gateway inherited BOOT from launcher: task=%s mission_type=%s; "
                "waiting for FCU/pose/FSM readiness",
                self.startup_boot_task,
                startup_mission_type,
            )
        ...
        rospy.Subscriber("/uav/drop_ack", String, self.servo_ack_cb, queue_size=20)
        ...
        rospy.logwarn(
            ...
            "FORCED_TASK=%s, STARTUP_BOOT_TASK=%s, FAKE_CAR_SEQUENCE=%s, "
            "POSE_MAX_AGE=%.2fs, STATUS_MAX_AGE=%.2fs",
            self.proto,
            self.listen_ip,
            self.listen_port,
            gs_mode,
            ...
            self.startup_boot_task or "NONE",
    ...
    def servo_ack_cb(self, msg):
        try:
            data = json.loads(msg.data)
        except Exception:
            return
        action_id = safe_int(data.get("action_id", 0), 0)
        # FSM 自动投放使用正 action_id；地面站手动命令使用负 action_id。
        if action_id >= 0:
            return
        with self.servo_result_condition:
            self.servo_results[action_id] = data
            self.servo_result_condition.notify_all()

    def next_manual_servo_action_id(self):
        self.manual_servo_action_id += 1
        return -self.manual_servo_action_id

    def wait_servo_result(self, action_id):
        deadline = time.monotonic() + self.servo_result_timeout
        with self.servo_result_condition:
            while action_id not in self.servo_results:
                remaining = deadline - time.monotonic()
                if remaining <= 0.0:
                    break
                self.servo_result_condition.wait(remaining)
            return self.servo_results.pop(action_id, None)

    def parse_servo_request(self, args):
        requested = str(args[0]).strip().upper() if args else ""
        aliases = {
            "LOCK": ("LOCK", self.servo_lock_command),
            "A30": ("LOCK", self.servo_lock_command),
            "L1": ("LOCK", self.servo_lock_command),
            "RELEASE": ("RELEASE", self.servo_release_command),
            "A90": ("RELEASE", self.servo_release_command),
            "R1": ("RELEASE", self.servo_release_command),
        }
    ...
    def learn_ground_station(self, addr, action):
        ...
        if self.strict_source_ip or not self.learn_ground_station_ip:
            return
        if action not in {"PING", "STATUS", "BOOT", "LAND", "RESET", "START", "SERVO"}:
            return
    ...
    def handle_command(self, text, addr):
        ...
        if self.strict_source_ip and role == "UNKNOWN":
            ...
            if role not in {"GS", "DEBUG"}:
                ...
                parsed_servo = self.parse_servo_request(args)
                if parsed_servo is None:
                    reply = self.err(cmd_id, action, "BAD_FORMAT", "USE_LOCK_OR_RELEASE")
                    ...
                    reply = self.err(
                        ...
                        action,
                        "BUSY",
                        self.fsm_state() or "FSM_NOT_WAIT_START",
                    )
                else:
                    semantic, serial_command = parsed_servo
                    action_id = self.next_manual_servo_action_id()
                    payload = {
                        "cmd": serial_command,
                        "mission_id": 0,
                        "action_id": action_id,
                    ...
                    self.servo_cmd_pub.publish(
                        String(data=json.dumps(payload, ensure_ascii=False, separators=(",", ":")))
                    )
                    result = self.wait_servo_result(action_id)
                    if result is None:
                        cacheable = False
                        reply = self.err(cmd_id, action, "NOT_READY", "ACTUATOR_NO_REPLY")
                    elif bool(result.get("success", False)):
                        reply = self.ack(cmd_id, action, "OK", semantic)
                        rospy.logwarn(
                            "Manual servo success: %s -> %s, reply=%s",
                            semantic,
                            serial_command,
    ...
    def parse_car_telemetry(self, text, addr):
        if not self.car_telemetry_enabled or self.car_pub is None:
            ...
            return
        role = self.source_role(addr)
        # 地面站 IP 可动态学习，但真车遥测必须始终来自配置的小车 IP。
        # 否则 strict_source_ip=false 时任意设备都能伪造 TEL:CAR。
        if role not in {"CAR", "DEBUG"}:
            rospy.logwarn_throttle(
                2.0,
                "TEL:CAR ignored from non-car source %s (configured car=%s)",
                str(addr[0]),
                self.car_ip,
            )
            return
        ...
        # 新版小车直接给出平台中心在场地坐标系中的速度分量。
        # 弯道中平台中心速度方向不等于车身 yaw，因此优先使用 vx/vy；
        # 旧版遥测没有这两个字段时，才退回 speed+yaw 兼容计算。
        has_vector_velocity = (
            data.get("vx_cm_s") is not None
            and data.get("vy_cm_s") is not None
        )
        if has_vector_velocity:
            vx_field = safe_float(data.get("vx_cm_s")) / 100.0
            vy_field = safe_float(data.get("vy_cm_s")) / 100.0
            speed = math.hypot(vx_field, vy_field)
        else:
        ...
        # COUNTDOWN/READY 只表示模式已选或安全撤离倒计时，绝不能让 FSM
        # 误认为小车已经运动。只有下列状态才视为实际运行。
        running_states = {
            "RUNNING", "PASS_B", "PASS_C", "PASS_D", "RETURN_A"
        }
        output = {
            "type": "car_state",
            "stamp": rospy.Time.now().to_sec(),
            ...
            "countdown_ms": safe_int(data.get("countdown_ms", 0), 0),
            "velocity_source": "vx_vy" if has_vector_velocity else "speed_yaw_fallback",
            "state": state,
    ...
    def parse_car_event(self, text, addr):
        ...
        if role not in {"CAR", "DEBUG"}:
            rospy.logwarn_throttle(
                2.0,
                "CAR event ignored from non-car source %s (configured car=%s)",
                str(addr[0]),
                self.car_ip,
            )
            return
