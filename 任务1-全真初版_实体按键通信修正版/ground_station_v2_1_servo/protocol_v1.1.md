...
```json
TEL:CAR:{"seq":310,"time_ms":18390,"run":"R001","task":1,"state":"RUNNING","x_cm":190.2,"y_cm":262.5,"speed_cm_s":28.5,"yaw_deg":91.3,"point":"B","line_detected":true,"battery":82,"error":0}
```
...
仅允许地面站在无人机未解锁且 FSM 处于 `WAIT_START` 时发送：
...
飞行中、已解锁或 FSM 不在 `WAIT_START` 时必须拒绝。正式任务投放由 FSM 自动发送 `A90`，地面站不能在飞行中手动控制。
