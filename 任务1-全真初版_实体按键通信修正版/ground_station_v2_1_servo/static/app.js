...
function renderMission(data) {
  ...
  const active = ["STARTING", "COUNTDOWN", "RUNNING", "ABORTING"].includes(status);
  ui.startBtn.disabled = true;
  const countdown = Number(mission.countdown_s || 0);
  if (status === "COUNTDOWN") {
    ui.startBtn.textContent = `安全撤离倒计时 ${Math.max(0, Math.ceil(countdown))} s`;
  } else if (status === "WAIT_BUTTON") {
    ui.startBtn.textContent = "请按小车 GPIO32 实体按键";
  } else if (status === "RUNNING") {
    ui.startBtn.textContent = "小车与无人机已启动";
  } else if (status === "DONE") {
    ui.startBtn.textContent = "任务已完成";
  } else if (status === "PREPARING") {
    ui.startBtn.textContent = "等待双方准备";
  } else {
    ui.startBtn.textContent = "实体按键启动（网页不可点击）";
  }
  ui.startBtn.title = mission.prompt || "地面站只负责模式选择，正式启动使用小车实体按键";
  ui.prepareBtn.disabled = active || Boolean(data.task_locked);
  setText("missionPrompt", mission.prompt || "--");
}
...
function renderDevice(device, data) {
  ...
  if (device === "uav") {
    ...
    setText("carSegment", tel.segment || "--");
    setText("carPath", fmt(tel.path_s_cm, 1, " cm"));
    setText("carCountdown", tel.state === "COUNTDOWN" ? fmt((Number(tel.countdown_ms || 0) / 1000), 1, " s") : "--");
...
function renderServoControls(data) {
  ...
  const missionActive = ["STARTING", "COUNTDOWN", "RUNNING", "ABORTING"].includes(missionStatus);
...
ui.startBtn.addEventListener("click", () => {
  showToast("正式启动请按小车 GPIO32 实体按键；网页不会发送 START");
});
