...
- UDP 同时与无人机和小车通信；
- 支持任务 1/任务 2、BOOT/MODE、PING、STATUS、实体按键启动流程、LAND、RESET；
...
class GroundStation:
    ...
    def build_settings_payload(self) -> Dict[str, Any]:
        ...
        busy = mission_status in {"STARTING", "COUNTDOWN", "RUNNING", "ABORTING"}
        return {
            "ok": True,
    ...
    def _handle_event(self, args: list[str], source: str) -> None:
        ...
        if event == "UAV_BOOT_READY":
            ...
            if self.state.get("task_locked") and str(self.state["car"]["telemetry"].get("state", "")).upper() == "READY":
                self.state["mission"].update({
                    "status": "WAIT_BUTTON",
                    "prompt": "双方已准备，请队员按下小车 GPIO32 实体按键",
                })
            ...
            self.state["mission"].update({
                "status": "WAIT_BUTTON",
                "countdown_s": 0,
                "prompt": "小车模式已确认，请队员按下 GPIO32 实体按键",
            })
        elif event == "CAR_COUNTDOWN":
            run_id = params[0] if len(params) >= 1 else self.state["mission"].get("run_id", "")
            task = params[1] if len(params) >= 2 else self.state.get("selected_task")
            try:
                countdown_s = max(0, int(float(params[2]))) if len(params) >= 3 else 10
            except (TypeError, ValueError):
                countdown_s = 10
            self.state["car"]["telemetry"].update({"state": "COUNTDOWN", "task": task})
            self.state["mission"].update({
                "run_id": run_id,
                "status": "COUNTDOWN",
                "countdown_s": countdown_s,
                "prompt": "安全撤离倒计时，倒计时结束后小车与无人机自动启动",
            })
        elif event == "CAR_COUNTDOWN_CANCELLED":
            self.state["car"]["telemetry"]["state"] = "READY"
            self.state["mission"].update({
                "run_id": "",
                "status": "WAIT_BUTTON",
                "countdown_s": 0,
                "prompt": "倒计时已取消，请重新按下小车实体按键",
            })
        elif event == "MISSION_START":
            run_id = params[0] if params else self.state["mission"].get("run_id", "")
            self.state["mission"].update(
                {"run_id": run_id, "status": "RUNNING", "countdown_s": 0,
                 "prompt": "任务运行中"}
            )
            ...
            self.state["mission"].update({
                "status": "DONE", "countdown_s": 0, "prompt": "任务完成"
            })
    ...
    def _handle_telemetry(self, device: str, payload: str) -> None:
        ...
        incoming_run_id = str(data.get("run", "")).strip()
        if incoming_run_id not in {"", "-", "R000", "NONE"}:
            self.state["mission"]["run_id"] = incoming_run_id

        if device == "car":
            car_state = str(data.get("state", "")).upper()
            countdown_ms = max(0, int(float(data.get("countdown_ms", 0) or 0)))
            if car_state == "COUNTDOWN":
                countdown_s = (countdown_ms + 999) // 1000
                self.state["mission"].update({
                    "status": "COUNTDOWN",
                    "countdown_s": countdown_s,
                    "prompt": "安全撤离倒计时，结束后自动启动",
                })
            elif car_state == "READY" and self.state.get("task_locked"):
                if str(self.state["mission"].get("status", "")).upper() not in {"RUNNING", "DONE", "ABORTING", "ABORTED"}:
                    self.state["mission"].update({
                        "status": "WAIT_BUTTON",
                        "countdown_s": 0,
                        "prompt": "请按下小车 GPIO32 实体按键",
                    })
            elif car_state in {"RUNNING", "PASS_B", "PASS_C", "PASS_D", "RETURN_A"}:
                self.state["mission"].update({
                    "status": "RUNNING", "countdown_s": 0, "prompt": "任务运行中"
                })
    ...
    async def api_settings_save(self, request: web.Request) -> web.Response:
        mission_status = str(self.state["mission"].get("status", "IDLE")).upper()
        if mission_status in {"STARTING", "COUNTDOWN", "RUNNING", "ABORTING"}:
            return web.json_response(
                {
                    "ok": False,
    ...
    async def api_debug_uav_command(self, request: web.Request) -> web.Response:
        """调试页专用：只向无人机发送白名单命令，不经过小车。"""
        body = await self._json_body(request)
        ...
        args: list[str] = []
        emergency = action == "LAND"
        run_id = ""

        if action in {"BOOT", "START"}:
            if task not in {"T1", "T2"}:
                return web.json_response(
                    {"ok": False, "error": f"{action} 必须选择 T1 或 T2"},
                    status=400,
                    dumps=lambda x: json.dumps(x, ensure_ascii=False),
                )
        ...
        if action == "BOOT":
            ...
            args = [run_id, task]
            self.state["selected_task"] = task
            self.state["mission"].update({"run_id": run_id, "status": "STARTING", "last_event": ""})
        elif action == "LAND":
            run_id = str(self.state["mission"].get("run_id") or "R000")
            args = [run_id]
            self.state["mission"]["status"] = "ABORTING"
            self.state["uav"]["telemetry"]["safety"] = "ABORTING"
        elif action == "RESET":
            self.state["mission"].update({
            "run_id": "", "status": "IDLE", "last_event": "",
            "countdown_s": 0, "prompt": "请选择任务并准备双方"
        })
        ...
        self.add_log(
            level,
            "DEBUG",
            f"无人机直发命令：CMD:{cmd_id}:{action}" + (f":{detail}" if detail else ""),
        )
        payload = {"ok": True, "cmd_id": cmd_id, "action": action}
        if run_id:
    ...
    async def api_servo(self, request: web.Request) -> web.Response:
        ...
        if armed or mission_status in {"STARTING", "COUNTDOWN", "RUNNING", "ABORTING"}:
            return web.json_response(
    ...
    async def api_prepare(self, request: web.Request) -> web.Response:
        ...
        self.state["mission"].update({
            "status": "PREPARING", "last_event": "", "countdown_s": 0,
            "prompt": "正在等待 UAV_BOOT_READY 和 CAR_READY"
        })
        self.state["uav"]["telemetry"].update({"boot": "STARTING", "state": "STARTING"})
    ...
    async def api_start(self, request: web.Request) -> web.Response:
        # 正式比赛流程由小车实体按键触发。地面站绝不向小车发送 START，
        # 避免远程误启动或与 10 秒安全撤离倒计时冲突。
        self.add_log("WARN", "GS", "网页 START 已禁用：请按小车 GPIO32 实体按键")
        return web.json_response(
            {"ok": False, "error": "请按小车 GPIO32 实体按键；地面站不发送 START"},
            status=410,
            dumps=lambda x: json.dumps(x, ensure_ascii=False),
        )
    ...
    @staticmethod
    def _no_cache_page(path: Path) -> web.FileResponse:
    ...
    async def index(self, request: web.Request) -> web.FileResponse:
        return self._no_cache_page(BASE_DIR / "templates" / "index.html")

    async def settings_page(self, request: web.Request) -> web.FileResponse:
        return self._no_cache_page(BASE_DIR / "templates" / "settings.html")

    async def debug_page(self, request: web.Request) -> web.FileResponse:
        return self._no_cache_page(BASE_DIR / "templates" / "debug.html")
...
def create_app(config: Dict[str, Any], config_path: Path) -> web.Application:
    ...
    app.router.add_get("/debug", station.debug_page)
