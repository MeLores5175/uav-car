...
"""投放机构串口桥：接收 /uav/drop_cmd JSON，幂等执行并发布 /uav/drop_ack。"""
...
class DropActuatorSerial:
    def __init__(self):
        ...
        self.timeout_s = float(acfg.get("timeout_s", 0.35))
        self.boot_wait_s = float(acfg.get("boot_wait_s", 2.0))
        self.retry_count = int(acfg.get("retry_count", 3))
        self.retry_delay_s = float(acfg.get("retry_delay_s", 0.20))
        ...
        self.ack_pub = rospy.Publisher("/uav/drop_ack", String, queue_size=10)
        rospy.Subscriber("/uav/drop_cmd", String, self.command_cb, queue_size=10)
        rospy.on_shutdown(self.shutdown)

        if self.enabled and not self.simulate:
            self.open_serial(force=True)
        rospy.logwarn(
            "Drop actuator bridge ready. enabled=%s simulate=%s port=%s baud=%d",
            str(self.enabled), str(self.simulate), self.port, self.baud
        )
        ...
        for path in self.serial_candidates():
            try:
                ...
                try:
                    ...
                    pass
                rospy.logwarn("Drop serial opened: %s @ %d", self.port, self.baud)
                return True
            except Exception as exc:
                errors.append("%s:%s" % (path, str(exc)))
        rospy.logerr_throttle(2.0, "Cannot open drop serial: %s", " | ".join(errors))
        return False
    ...
    def publish_ack(self, mission_id, action_id, success, reply, attempts=0):
        payload = {
    ...
    def command_cb(self, msg):
        try:
            ...
            data = {"cmd": msg.data.strip(), "mission_id": 0, "action_id": 0}

        command = str(data.get("cmd", "")).strip()
        mission_id = int(data.get("mission_id", 0))
        action_id = int(data.get("action_id", 0))
        if not command:
            self.publish_ack(mission_id, action_id, False, "EMPTY_COMMAND", 0)
            return

        key = (mission_id, action_id)
        with self.lock:
            if key in self.successful_actions:
                self.publish_ack(mission_id, action_id, True, "DUPLICATE_ALREADY_DONE", 0)
                return
            ok, reply, attempts = self.execute_serial(command)
            if ok:
                self.successful_actions.add(key)
            self.publish_ack(mission_id, action_id, ok, reply, attempts)
            if ok:
                rospy.logwarn(
                    "Drop action success mission=%d action=%d cmd=%s reply=%s",
                    mission_id, action_id, command, reply
                )
            else:
                rospy.logerr(
                    "Drop action failed mission=%d action=%d cmd=%s reply=%s",
                    mission_id, action_id, command, reply
                )
