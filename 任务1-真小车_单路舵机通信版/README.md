...
本版本已经把单路 ESP32 舵机接入任务一完整链路：

- ESP32 上电自动执行 `A30`，作为锁止位置；
- 地面站起飞前可发送 `CMD:<id>:SERVO:LOCK`，无人机写串口 `A30`；
- 地面站起飞前可发送 `CMD:<id>:SERVO:RELEASE`，无人机写串口 `A90`；
- 手动舵机命令只在 FCU 未解锁且 FSM 为 `WAIT_START` 时接受；
- FSM 满足投放条件后自动通过 `/uav/drop_cmd` 请求 `A90`；
- `drop_actuator_serial.py` 是唯一打开 ESP32 串口的节点，执行后通过 `/uav/drop_ack` 返回结果，避免多个节点抢占串口。

串口参数在 `config/air_ground_mission.yaml` 的 `actuator` 段设置。默认：
...
```text
esp32/esp32_c3_single_servo_A30_A90.ino
```
